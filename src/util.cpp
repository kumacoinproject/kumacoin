...
inline int OutputDebugStringF(const char* pszFormat, ...)
{
    ...
    else if (!fPrintToDebugger)
    {
        ...
        if (!fileout)
        {
            fileout = fopen(GetDebugLogFilePath().string().c_str(), "a");
        ...
        if (fileout)
        {
            ...
            if (fReopenDebugLog) {
                ...
                if (freopen(GetDebugLogFilePath().string().c_str(),"a",fileout) != NULL)
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "KumaCoind.pid"));
...
void ShrinkDebugFile()
{
    ...
    boost::filesystem::path pathLog = GetDebugLogFilePath();
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong KumaCoin will not work properly.");
                    strMiscWarning = strMessage;
                    printf("*** %s\n", strMessage.c_str());
                    uiInterface.ThreadSafeMessageBox(strMessage+" ", string("KumaCoin"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION);
                }
            }
        }
        if (fDebug) {
